...
var tabnav_padding_start = parseInt($('#centreColumn .tabnav').css(rightOptions_padding_side));
...
function responsive_update() {
	...
	//move alerts
  	if(globalWindowWidth < 768){
		...
		if($('#centreColumn .tabnav').siblings('.optionsRight').length && $('#centreColumn .tabnav').parent('.inline_tabs_options').length == 0) {
				$('#centreColumn .tabnav').css(rightOptions_padding_side, '7px');
			}
			...
			if($('#centreColumn .tabnav').siblings('.optionsRight').length && $('#centreColumn .tabnav').parent('.inline_tabs_options').length == 0) {
				$('#centreColumn .tabnav').css(rightOptions_padding_side, $('#centreColumn .tabnav').siblings('.optionsRight').outerWidth() + tabnav_padding_start);
			}
  	}
  ...
  if($('.pageHeading').length > 1 && $('#fixedSectionHeader').length == 0){
	...
	$('.tabsRight').each(function(){
		if($(this).siblings('.tabnav').length){
			$(this).siblings('.tabnav').css(rightOptions_padding_side, $(this).outerWidth());
		}else if($(this).parent().siblings('.tabnav').length){
			$(this).parent().siblings('.tabnav').css(rightOptions_padding_side, $(this).outerWidth());
		}
	});
	...
	if(!page_title_updated){
		if(typeof title_text_override != 'undefined') {
			updatePageTitle(title_text_override);
		} else {
			updatePageTitle();
		}
...
function options_ribbon_adjustment() {
	if ($('.optionsRibbon').length) {
		...
		$('.optionsRibbon').each(function() {
			const $items = $(this).find('li:not(.mobile_only)');
			const $visibleItems = $items.filter(function() { return this.getClientRects().length > 0; });
			const alreadyAdjusted = $items.filter('.overflowOption').length > 0;

			// Skip dropdown layout work when all ribbon actions already fit on one line.
			if ($visibleItems.length && !alreadyAdjusted && $visibleItems.last().offset().top <= $visibleItems.first().offset().top) {
				$(this).find('li.mobile_only').attr('style', 'display: none');
				return;
			}
...
function responsive_cell_headers(table) {
  try {
    ...
    for (var i = 0; i < ths.length; i++) {
      ...
      if (ths[i].getElementsByTagName('a').length > 0) {
        headingText = ths[i].getElementsByTagName('a').item(0).innerHTML;
      }
    ...
    var randomClass = 'rc' + Math.random().toString(36).substring(7); // when generated if first letter is numeric, insertRule throws error so appending rc.
