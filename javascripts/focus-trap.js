...
// so that it precedes the focus event
  ...
  updateTabbableNodes: function() {
    var temp = [];
    var tabbableSelector = 'a[href], area[href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), iframe, object, embed, [tabindex]:not([tabindex="-1"]), [contenteditable]';
    
    $(focusTrap.trap).find(tabbableSelector).each(function(){
      if ($(this).is(':visible') || $(this).attr('id') === 'content_ifr') {
        temp.push($(this)[0]);
      }
    });
